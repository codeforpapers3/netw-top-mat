...
lemma loop_matrix_rlc: 
  ...
  assumes nodes: "\<N>s = [n1::'a, n2, n3, n4]" and edges: "\<E>s = [(n4,n1), (n1,n2), (n1,n3), (n3,n2), (n2,n4), (n3,n4)]"
   and loops: "\<L>s = [[(n4,n1),(n1,n2),(n2,n4)], 
                     [(n3,n4),(n4,n2),(n2,n3)], 
                     [(n1,n3),(n3,n2),(n2,n1)], 
                     [(n4,n1),(n1,n2),(n2,n3),(n3,n4)], 
                     [(n4,n1),(n1,n3),(n3,n4)],
                     [(n2,n1),(n1,n3),(n3,n4),(n4,n2)], 
                     [(n4,n1),(n1,n3),(n3,n2),(n2,n4)]]"  
 shows "(C :: real mat)  = mat_of_rows_list 6 [[1::real, 1, 0, 0, 1, 0],
                                               [0::real, 0, 0, -1, -1, 1], 
                                               [0::real, -1, 1, 1, 0, 0], 
                                               [1::real, 1, 0, -1, 0, 1],
                                               [1::real, 0, 1, 0, 0, 1],
                                               [0::real, -1, 1, 0, -1, 1], 
                                               [1::real, 0, 1, 1, 1, 0]]"  (is "_ = ?rhs")
proof(rule eq_matI)
  have 0: "distinct \<N>s" 
  ...
  have pos22b: "(\<E>s!2)\<in> set (\<L>s!2)" using assms 6  by auto
  then have pos22: "in_pos_loop (\<E>s!2) (\<L>s!2)" using assms in_pos_loop
    by (metis (no_types, lifting) "2" "3" C_dimcol_is_K_dimcol C_matrix_edge_in_revloop dim_col_K dim_row_is_l inneg_loop_C_mone 
          le_neq_implies_less nle_le not_in_loop_indexed numeral_le_iff semiring_norm(70) semiring_norm(71) semiring_norm(72))
    have pos23b: "(\<E>s!3)\<in> set (\<L>s!2)"  using assms by auto
  then have pos23: "in_pos_loop (\<E>s!3) (\<L>s!2)" using assms in_pos_loop 
    by (metis "2" "3" dim_col_C inpos_loop_C_one loop_matrow_dim nonempty_loop_system.C_matrix_pos_iff nonempty_loop_system_axioms numeral_less_iff semiring_norm(68) semiring_norm(77) semiring_norm(79) semiring_norm(81))   
  ...
  then have neg21: "in_neg_loop (\<E>s!1) (\<L>s!2)" 
    using assms in_neg_loop 2 3 edge_rloop1 
    by (metis Suc_1 Suc_mono dim_col_C dim_row_is_l eval_nat_numeral(3) in_loop_not_oppositeneg_directed not_inpos_inneg_loop' nth_mem numeral_One numeral_less_iff semiring_norm(76) wf_loop_system wf_pos_loop)
