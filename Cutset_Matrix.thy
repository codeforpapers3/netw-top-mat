...
lemma cutset_vec_index_mone_alt: 
  assumes "i < length Cs" and "length N = length Cs" and  "(Cs!i) =  (cutset (N!i))"
  shows  "cutset_vec Cs p $ i = -1 \<longleftrightarrow>  p \<in> set (Cs ! i) \<and>  p \<in> set (neg_cut_list (N!i))"
  using assms neg_cutset_cond_indexed cutset_vec_index_mone_negc 
  by (metis invalid_cut_pos_neg neg_cut_not_pos neg_cutset_altdef pos_cutset_altdef wf_invalid_cs)
...
lemma cutset_vec_index_one_alt: 
  ...
  by (metis (mono_tags, lifting) neg_cut_not_pos neg_cutset_altdef nth_mem wf_invalid_edge)
...
lemma cutset_vec_index_zero_alt: 
  ...
  by (smt (verit) cutset_def cutset_vec_index_one cutset_vec_index_one_alt mem_Collect_eq set_filter)
...
lemma cutset_rvec_index_mone: "j < length Es  \<Longrightarrow> cs = cutset u \<Longrightarrow> (Es!j) \<notin> set (pos_cut_list u) \<Longrightarrow> 
        cutset_rvec u cs Es $ j = -1  \<longleftrightarrow> (Es!j) \<in> set (neg_cut_list u) \<and> (Es ! j) \<in> set cs"
  using C\<^sub>n_def C\<^sub>p_def in_pos_cutset_def in_neg_cutset_def
  by (metis (no_types, lifting) cutset_rvec_index invalid_cut_pos_neg neg_cutset_altdef pos_cutset_altdef wf_invalid_cs zero_neq_neg_one) 

lemma cutset_rvec_index_one: "j < length Es  \<Longrightarrow> cs = cutset u \<Longrightarrow> (Es!j) \<notin> set (neg_cut_list u) \<Longrightarrow>
      cutset_rvec u cs Es $ j = 1  \<longleftrightarrow> (Es!j) \<in> set (pos_cut_list u) \<and> (Es ! j) \<in> set cs"
 using in_pos_cutset_def in_neg_cutset_def
proof -
  assume a1: "Es ! j \<notin> set (neg_cut_list u)"
  assume a2: "cs = cutset u"
  assume "j < length Es"
  then have f3: "nonempty_cutset_system \<N>s \<E>s \<C>s \<and> j < length Es"
    using nonempty_cutset_system_axioms by blast
  have f4: "u \<in> set N \<longrightarrow> cutset u \<in> \<C>"
    using wf_2 by blast
  have "\<not> in_neg_cutset u (Es ! j) cs \<or> Es ! j \<in> set (neg_cut_list u)"
    using neg_cutset_altdef by blast
  then have f5: "\<not> in_neg_cutset u (Es ! j) cs"
    using a1 by force
  have "Es ! j \<in> set cs \<or> \<not> in_pos_cutset u (Es ! j) cs"
    using notin_cutsets_not_pos_not_neg by blast
  then show ?thesis
    using f5 f4 f3 a2 
    by (metis (no_types, lifting) cutset_edges cutset_rvec_index nonempty_cutset_system.pos_cutset_altdef nonempty_network_system.pos_cut_list_alt one_neq_zero wf_nonempty_netw_list_sys)
qed

lemma cutset_rvec_index_zero: "j < length Es  \<Longrightarrow> cs = cutset u \<Longrightarrow> cutset_rvec u cs Es $ j = 0 \<longleftrightarrow> (Es ! j) \<notin> set cs "
  ...
  by (metis (no_types, lifting) add.inverse_neutral cut_list_pos_neg cutset_edges mem_Collect_eq neg_cutset_altdef nonempty_network_system.pos_cut_not_neg pos_cutset_altdef set_filter zero_neq_neg_one)
...
lemma cutset_matrix_edge_in_csneg_mone_alt:
  ...
  by (smt (verit, best) assms case_prod_conv index_mat(1) neg_cut_not_pos pos_cutset_altdef wf_invalid_cs)
...
lemma cutset_matrix_edge_in_cspos_one_alt:
  ...
  (Es!j) \<in> set (Cs ! i) \<and> (Es!j) \<in> set (pos_cut_list (N!i))
    \<Longrightarrow> (cutset_matrix Cs Es) $$ (i, j) = 1"
  using wf_nonempty_netw_list_sys pos_cutset_altdef pos_cut_list_alt
by (metis (no_types, lifting) assms cutset_matrix_edge_in_cspos_one neg_cut_not_pos neg_cutset_altdef wf_invalid_cs)
...
lemma B_matrix_edge_notin_cutset: "i < z \<Longrightarrow> j < n \<Longrightarrow> (\<C>s!i) = (cutset (N!i)) \<Longrightarrow> B $$ (i, j) = 0
\<Longrightarrow> (\<E>s ! j) \<notin> set (\<C>s ! i) "
  using nonempty_cutset_system.cutset_matrix_edge_notin_cs_iff 
 nonempty_cutset_system_axioms not_pos_nor_neg_ntin_cuts_cond_indexed by blast

lemma B_matrix_edge_in_negcutset_iff: "i < z \<Longrightarrow> j < n \<Longrightarrow> (\<C>s!i) = (cutset (N!i)) \<Longrightarrow> 
  B $$ (i, j) = -1 \<longleftrightarrow> (\<E>s ! j) \<in> set (\<C>s ! i) \<and> (\<E>s ! j) \<in> set (neg_cut_list (N!i))"
...
lemma B_matrix_edge_notin_cutset_iff: "i < z \<Longrightarrow> j < n  \<Longrightarrow> (\<C>s!i) = (cutset (N!i)) \<Longrightarrow> B $$ (i, j) = 0
\<Longrightarrow>  (\<E>s ! j) \<notin> set (\<C>s ! i) "
...
lemma cutset_mat_degree: "i < length  \<C>s \<Longrightarrow> of_nat (mat_degree_num B i) = sum_abs_vec (row B i)"
  ...
  by (meson B_mat_row_elems count_abs_vec1_sum_non_zero_elems_alt)
