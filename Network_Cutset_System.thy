...
lemma proper_subplist_alt1: 
  fixes E :: "'a edges" 
  assumes "distinct E" and "es \<in> set (subseqs E)"
  shows " es \<in> set (proper_sub_plist E) \<Longrightarrow> es \<noteq> [] \<and> es \<noteq> E "
  using assms
  by (simp add: proper_sub_plist_def)
...
Then, a cut is defined as the set of edges and each of which is incident with one of its two endpoints in V1 and the other in V2
\<close>
...
lemma in_pos_cutset: assumes "e \<in> set \<E>s" assumes "cs \<in> set \<C>s" assumes "e \<in> set cs"
  shows "in_pos_cutset u e cs \<longleftrightarrow>  e \<in> set (pos_cut_list u) \<and> e \<in> set (cutset u)"
  using assms(2) assms(3) pos_cut_list_alt pos_cutset_altdef by blast
